...
if [[ -z "${OPENSEARCH_INITIAL_ADMIN_PASSWORD:-}" ]]; then
  ...
  echo "  export OPENSEARCH_INITIAL_ADMIN_PASSWORD='<set-a-strong-secret>'"
  exit 1
fi
if [[ -z "${KAFKA_ADMIN_PASSWORD:-}" || -z "${KAFKA_CLIENT_PASSWORD:-}" ]]; then
  ...
  echo "  export KAFKA_ADMIN_PASSWORD='<set-a-strong-secret>'"
  echo "  export KAFKA_CLIENT_PASSWORD='<set-a-strong-secret>'"
  exit 1
fi
